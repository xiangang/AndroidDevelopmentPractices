...
import com.nxg.srollview.utils.ReflectUtils;

import java.util.List;

/**
 * 无限循环的LinearLayoutManager
 */
public class LooperLinearLayoutManager extends RecyclerView.LayoutManager implements
     ...
     */
    public LooperLinearLayoutManager(Context context) {
     ...
     */
    public LooperLinearLayoutManager(Context context, @RecyclerView.Orientation int orientation,
     ...
     */
    public LooperLinearLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                                     int defStyleRes) {
     ...
     * androidx.recyclerview.widget.LinearLayoutManager} and stores the result in {@code extraLayoutSpace}. {@code
    ...
    private void layoutForPredictiveAnimations(RecyclerView.Recycler recycler,
        ...
        for (int i = 0; i < scrapSize; i++) {
            ...
            //使用反射获取mFlags，进行(mFlags & FLAG_REMOVED) != 0计算代替isRemoved
            Object flagObj = ReflectUtils.reflect(scrap).field("mFlags").get();
            if (flagObj != null) {
    ...
    private void updateLayoutState(int layoutDirection, int requiredSpace,
        ...
        int scrollingOffset;
        //滚动方向(手往上上滑动)列表的尾部（竖直方向：对应底部，水平方向：对应右侧）向头部（竖直方向：对应顶部，水平方向：对应左侧）滚动
        if (layoutToEnd) {
            //如果想要预先布局不可见的Item则使用这个mExtraFillSpace参数（额外填充空间）
            mLayoutState.mExtraFillSpace += mOrientationHelper.getEndPadding();
            // get the first child in the direction we are going
            //从尾部获取往滚动方向起的第一个（起点最近的）item
            final View child = getChildClosestToEnd();
            // the direction in which we are traversing children
            //准备遍历的item的方向（这里要考虑是否翻转，由构造函数中的reverseLayout参数决定）
            mLayoutState.mItemDirection = mShouldReverseLayout ? LayoutState.ITEM_DIRECTION_HEAD
                    : LayoutState.ITEM_DIRECTION_TAIL;
            //获取从尾部获取往滚动方向起的第一个（起点最近的）item的位置
            mLayoutState.mCurrentPosition = getPosition(child) + mLayoutState.mItemDirection;
            //计算从尾部获取往滚动方向起的第一个（起点最近的）item的在滚动反方向上的边界的像素值，包含decoration和margin
            mLayoutState.mOffset = mOrientationHelper.getDecoratedEnd(child);
            // calculate how much we can scroll without adding new children (independent of layout)
            //计算从尾部往滚动方向起的第一个（起点最近的）item完全显示出来还有多长的距离（如果这个item完全显示，意味着下一个item就呼之欲出，所以要提前add到布局中）
            //scrollingOffset实在mOffset的基础上减去（item的减去padding后的边界值）
    ...
    int fill(RecyclerView.Recycler recycler, LayoutState layoutState,
        ...
        //需要填充的像素值，应该是等于屏幕上RecyclerView的可用空间（等于RecyclerView的空间减去已经显示的所有Item的空间之和）
        final int start = layoutState.mAvailable;
        //如果滚动的偏移量不等于Integer.MIN_VALUE;
        ...
        //剩余空间 = RecyclerView的可用空间 + 预布局未显示的Item的占用的空间
        int remainingSpace = layoutState.mAvailable + layoutState.mExtraFillSpace;
        //避免被分配的空间，layoutChunk的结构
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        View firstOrLastView;
        if (getItemCount() > 0 && (layoutState.mCurrentPosition < 0 || layoutState.mCurrentPosition >= getItemCount())) {
            //这里要判断下滚动方向
    ...
    boolean shouldMeasureTwice() {
        ...
        Object hasFlexibleChildInBothOrientationsObj = ReflectUtils.reflect(this).method("hasFlexibleChildInBothOrientations").get();
        Log.i(TAG, "shouldMeasureTwice: hasFlexibleChildInBothOrientationsObj = " + hasFlexibleChildInBothOrientationsObj);
        if (hasFlexibleChildInBothOrientationsObj == null) {
            return false;
        }
        return getHeightMode() != View.MeasureSpec.EXACTLY
                && getWidthMode() != View.MeasureSpec.EXACTLY
                && (boolean) hasFlexibleChildInBothOrientationsObj;
    }
    ...
    View findOneVisibleChild(int fromIndex, int toIndex, boolean completelyVisible,
        ...
        /*//反射获取mHorizontalBoundCheck和mVerticalBoundCheck对象
        Object mHorizontalBoundCheckObj = ReflectUtils.reflect(this).field("mHorizontalBoundCheck").get();
        Object mVerticalBoundCheckObj = ReflectUtils.reflect(this).field("mVerticalBoundCheck").get();
        if (mHorizontalBoundCheckObj != null && mVerticalBoundCheckObj != null) {
            ViewBoundsCheck mHorizontalBoundCheck = (ViewBoundsCheck) mHorizontalBoundCheckObj;
            ViewBoundsCheck mVerticalBoundCheck = (ViewBoundsCheck) mVerticalBoundCheckObj;
            return (mOrientation == HORIZONTAL) ? mHorizontalBoundCheck.findOneViewWithinBoundFlags(fromIndex, toIndex, preferredBoundsFlag,
    ...
    View findOnePartiallyOrCompletelyInvisibleChild(int fromIndex, int toIndex) {
        ...
        @com.nxg.srollview.widget.ViewBoundsCheck.ViewBounds int preferredBoundsFlag = 0;
        @com.nxg.srollview.widget.ViewBoundsCheck.ViewBounds int acceptableBoundsFlag = 0;
        if (mOrientationHelper.getDecoratedStart(getChildAt(fromIndex))
                < mOrientationHelper.getStartAfterPadding()) {
            preferredBoundsFlag = (com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVS_LT_PVS | com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVE_LT_PVE
                    | com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVE_GT_PVS);
            acceptableBoundsFlag = (com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVS_LT_PVS
                    | com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVE_LT_PVE);
        } else {
            preferredBoundsFlag = (com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVE_GT_PVE | com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVS_GT_PVS
                    | com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVS_LT_PVE);
            acceptableBoundsFlag = (com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVE_GT_PVE
                    | com.nxg.srollview.widget.ViewBoundsCheck.FLAG_CVS_GT_PVS);
        }
       /* //反射获取mHorizontalBoundCheck和mVerticalBoundCheck对象
        Object mHorizontalBoundCheckObj = ReflectUtils.reflect(this).field("mHorizontalBoundCheck").get();
        Object mVerticalBoundCheckObj = ReflectUtils.reflect(this).field("mVerticalBoundCheck").get();
    ...
    private final ViewBoundsCheck.Callback mHorizontalBoundCheckCallback =
            new ViewBoundsCheck.Callback() {
                ...
                public View getChildAt(int index) {
                    return LooperLinearLayoutManager.this.getChildAt(index);
                }
                ...
                public int getChildStart(View view) {
                    ...
                    return LooperLinearLayoutManager.this.getDecoratedLeft(view) - params.leftMargin;
                }
                ...
                public int getChildEnd(View view) {
                    final RecyclerView.LayoutParams params = (RecyclerView.LayoutParams)
                            view.getLayoutParams();
                    return LooperLinearLayoutManager.this.getDecoratedRight(view) + params.rightMargin;
                }
            };
    ...
    private final ViewBoundsCheck.Callback mVerticalBoundCheckCallback =
            new ViewBoundsCheck.Callback() {
                ...
                @Override
                public int getParentEnd() {
                    return LooperLinearLayoutManager.this.getHeight()
                            - LooperLinearLayoutManager.this.getPaddingBottom();
                }
                ...
                public int getChildStart(View view) {
                    final RecyclerView.LayoutParams params = (RecyclerView.LayoutParams)
                            view.getLayoutParams();
                    return LooperLinearLayoutManager.this.getDecoratedTop(view) - params.topMargin;
                }
                ...
                public int getChildEnd(View view) {
                    final RecyclerView.LayoutParams params = (RecyclerView.LayoutParams)
                            view.getLayoutParams();
                    return LooperLinearLayoutManager.this.getDecoratedBottom(view) + params.bottomMargin;
                }
            };
     ...
     * {@link LooperLinearLayoutManager#findOneVisibleChild(int, int, boolean, boolean)},
     * and {@link LooperLinearLayoutManager#findOnePartiallyOrCompletelyInvisibleChild(int, int)}.
     */
