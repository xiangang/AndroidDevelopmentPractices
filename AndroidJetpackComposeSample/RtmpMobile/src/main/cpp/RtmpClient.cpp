...
int RtmpClient::init(std::string url, int w, int h, int time_out) {

    RTMP_LogSetLevel(RTMP_LOGDEBUG);
    rtmp = RTMP_Alloc();
    RTMP_Init(rtmp);
    LOGI("time out = %d", time_out);
...
int RtmpClient::sendVideoData(BYTE *buf, int len, long timestamp) {
    ...
    auto *packet = (RTMPPacket *) malloc(RTMP_HEAD_SIZE + len + 9);
    memset(packet, 0, RTMP_HEAD_SIZE);
    packet->m_body = (char *) packet + RTMP_HEAD_SIZE;
...
int RtmpClient::sendAudioSpec(BYTE *data, int spec_len) {
    if (!this->rtmp) return 0;
    RTMPPacket *packet;
    BYTE *body;
...
int RtmpClient::sendAudioData(BYTE *data, int len, long timestamp) {
    if (!this->rtmp) return 0;
//    data += 5;
//    len += 5;
    if (len > 0) {
