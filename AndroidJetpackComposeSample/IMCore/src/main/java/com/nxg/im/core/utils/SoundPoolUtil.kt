...
class SoundPoolUtil private constructor(val context: Context) {
    ...
    private var playMaps = mutableMapOf<Any, Int>()
    ...
    private fun play(
        ...
        spool?.play(
            ...
            ?.let {
                Log.i(TAG, "play: streamId $streamId -> playId $it")
                playMaps[streamId] = it
            }
    }
    ...
    fun stop(soundResId: Int) {
        //停止音频
        streamMaps[soundResId]?.let { streamId ->
            playMaps[streamId]?.let { playId ->
                Log.i(TAG, "play: streamId $streamId -> playId $playId")
                spool?.stop(playId)
            }
